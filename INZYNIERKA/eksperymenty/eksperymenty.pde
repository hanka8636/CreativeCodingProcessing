...
void drawViz(float y, int wl) {
  for (int i = 0; i < wl; i++)
  {
   ...
   float d1 = 10 + (sin(fftAlpha) * step) + fftAlpha/2;
   pushMatrix();
  /// translate(width/2, height/2);
  // rotate(d1);
  // line(xStart + (i* xSpacing),y,xStart + (i* xSpacing)+d1, y+d1);
  ...
  fftAlpha += jitter;
  float c = cos(fftAlpha);//pushMatrix();
  translate(xStart + (i* xSpacing)/2, y/2);
  rotate(c);
  rect(xStart + (i* xSpacing), y, 25, 25);  
  popMatrix();
    println(d1);
  }
